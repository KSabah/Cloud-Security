        ...
        if b == 0:
            for entry in dbx.files_list_folder("").entries:
                if entry.name == group_name:
                    ...
                    members = [add_member] 
                    dbx.sharing_add_folder_member(entry.shared_folder_id, members,)
...
def RemoveMember(group_name):
    if(os.path.isfile("Groups/"+group_name+".txt")):
        ...
        for entry in dbx.files_list_folder("").entries:
                if entry.name == group_name:
                    member_selector = dropbox.sharing.MemberSelector.email(email)
                    dbx.sharing_remove_folder_member(entry.shared_folder_id, member_selector, leave_a_copy = False)
            ...
            if file.name == group_name+"encrypted_key.txt":
                ...
                with open(file.name, "rb") as f:
                    ...
                    for memberdata in data:
                        ...
                        enc_data = fernet.encrypt(memberdata)
                f.close()
...
def UploadFile(group_name, upload_file):
...
def DownloadFile(group_name, my_email):
    if (os.path.isfile("Groups/"+group_name+".txt")):
        b = 0
        with open("Groups/"+group_name+".txt", 'r') as f:
            emails = [line.strip() for line in f]
...
def main():
    ...
    if remove == "yes":
        ...
        RemoveMember(group_name)

    upload = raw_input("Do you want to upload a file? ")
    upload = upload.lower()
    if upload == "yes":
        group_name = raw_input("Which group would you like to upload to? ")
        upload_file = raw_input("What is the name of the file you'd like to upload? ")
        UploadFile(group_name, upload_file)

    download = raw_input("Do you want to download a file? ")
    download = download.lower()
    if download == "yes":
        group_name = raw_input("What group do you wanna download from? ")
        my_email = raw_input("What's your email? ")
        DownloadFile(group_name, my_email)
